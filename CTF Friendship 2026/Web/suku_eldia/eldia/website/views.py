    ...
    if request.method == "POST":
        if "file" in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                db.session.commit()
                flash("Permintaan Verifikasi Terkirim!", category="success")

            elif file and not allowed_file(file.filename):
                flash("Hanya diperbolehkan format png, jpg, jpeg!", category="error")
    
    query = Verification.query.filter_by(user_id=current_user.id).first()
    if query:
        if query.verified == 0:
            flash("Permintaan Anda telah dikirim ke Danchou Erwin, tunggu persetujuan.", category="error")
            return redirect(url_for("views.home"))
        
        elif query.verified == 1:
            flash("Sudah terverifikasi!", category="error")
...
def dashboard():
    ...
    if query :
        if query.verified == 0:
            flash("Dokumen identitas telah dikirim! Tunggu persetujuan", category="error")
            return redirect(url_for("views.home"))
        else:
...
def changepasswd(Hash):
    ...
    if query:
        if request.method == "POST":
            ...
            query1.password = generate_password_hash(new_password)
            db.session.commit()
        ...
        flash("Tautan reset kata sandi tidak valid atau sudah kedaluwarsa!", category="error")
        return redirect(url_for("views.home"))
...
def forgetpassword():
    if request.method == "POST":
        ...
        flash("Jika email terdaftar, Anda akan mendapatkan tautan untuk mereset kata sandi!", category="success")
        if query:
...
def admin():
    if current_user.username == "admin":
        ...
        flash("Anda bukan Danchou Erwin!", category="error")
        return redirect(url_for("views.home"))
...
def verify():
    if current_user.username == "admin":
        if request.method == "POST":
            ...
            if not query:
                flash("Permintaan tidak ditemukan!", category="error")
            ...
            if "Artist" in exif_table.keys():
                ...
                return render_template_string("Terverifikasi! {}".format(sec_code))
            else:
                return render_template_string("Tidak Terverifikasi! :(")
    else:
        flash("Terjadi kesalahan", category="error")
        return redirect(url_for("views.home"))
    ...
    if query:
        ...
        if request.method == "POST":
            ...
            if not checkInput(bio):
                query.bio = bio
                db.session.commit()
                flash("Bio berhasil diperbarui!", category="success")
                return render_template("profile.html", user=current_user, Bio=bio, User=User)
            else:
                flash("Hindari Karakter Terlarang!", category="error")

        return render_template("profile.html", user=current_user, Bio=query.bio, User=User)
    else:
        flash("Kirimkan lencana Anda untuk verifikasi!", category="error")
        return redirect(url_for("views.verification"))
